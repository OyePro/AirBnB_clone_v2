...
class TestConsole(unittest.TestCase):
    ...
    @unittest.skipIf(db == "db", "Testing file_storage only")
    def test_show(self):
        '''
        ...
        user_id = self.capt_out.getvalue().strip()
        sys.stdout = self.backup
        self.capt_out.close()
        self.capt_out = StringIO()
        sys.stdout = self.capt_out
        console.onecmd("show User " + user_id)
        x = self.capt_out.getvalue().strip()
    ...
    @unittest.skipUnless(db != "db", "Testing filestorage only")
    def test_show_class_name(self):
        '''
